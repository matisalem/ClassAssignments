...
public class LeaderElection {
    ...
    public synchronized Vote lookForLeader() {
        try {
            ...
            logger.info("Encuentra al logger");
            logger.info("Starting lookForLeader() for " + servidor.getServerId());
            sendNotifications();
            ...
            while(true){
                ...
                try {
                    ...
                    //               logger.info("Message polled: " + (message == null ? "null" : message.toString()));
                ...
                if (servidor.getPeerState() == ServerState.OBSERVER) {

                    Vote observerVote = new Vote(notification.getProposedLeaderID(), notification.getPeerEpoch());
                    servidor.setCurrentLeader(observerVote);
                    logger.info("Observer updated leader to: " + notification.getProposedLeaderID());
                    return observerVote;
                }
                    ...
                    if (haveEnoughVotes(votes, new Vote(proposedLeader, proposedEpoch))) {
                        ...
                        while (newMessage != null) {
                            ...
                            if (supersedesCurrentVote(newNotification.getProposedLeaderID(), newNotification.getPeerEpoch())){
                                ...
                                sendNotifications();
                                hayMejor = true;
                                logger.info("SOYA: " + servidor.getPeerState());
    ...
    private Vote acceptElectionWinner(ElectionNotification n) {
        ...
        logger.info("Leader accepted: " + vote.getProposedLeaderID() + ", Epoch: " + vote.getPeerEpoch());
        return vote;
    }
