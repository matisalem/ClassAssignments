...
public class UDPMessageReceiver extends Thread implements LoggingServer {
    ...
    private LinkedBlockingQueue<Message> gossips;
    ...
    public UDPMessageReceiver(LinkedBlockingQueue<Message> incomingMessages, LinkedBlockingQueue<Message> gossips, InetSocketAddress myAddress, int myPort, PeerServer peerServer) throws IOException {
        this.incomingMessages = incomingMessages;
    ...
    public void run() {
        ...
        while (!this.isInterrupted()) {
            try {
                ...
                if(gossips != null && received.getMessageType() == MessageType.GOSSIP){
                    gossips.offer(received);
                    continue;
                }
